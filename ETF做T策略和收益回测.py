...
def way_1(data):
    ...
    for i in range(1, len(data)):
        if data[i] <= next_buy:     # 如果当天价格符合买入条件
            buy.append(data[i])
            buy = sorted(buy)
            costs.append(sum(buy))  # 本次持仓的成本单价*len(costs)
            buy_count += 1          # 买入次数+1
    ...
    print('历史全部套牢金额：%s' % [round(v * 100, 1) for v in sorted(costs) if v > 0])
    print('平均套牢金额：%d' % (sum(costs)//len(costs) * 100))
    print('\033[1m最大套牢金额：%d\033[0m' % (max(costs) * 100))
    ...
    print('\033[1m期末清仓收益率：%.2f%%\033[0m' % round(100 * (end_total_market + earning - end_total_cost) / end_total_cost, 2))
...
if __name__ == "__main__":
    data=get_fund_data('512200', per=49, sdate='2019-06-01', edate='2020-06-01')
    # 修改数据类型
